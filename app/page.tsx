...
export default function Home() {
  ...
  const domain =
    toolOutput?.result?.structuredContent?.domain || toolOutput?.domain;
  ...
  return (
    <div
      className={clsx("font-sans relative")}
      style={{
        maxHeight,
    ...
    >
      {displayMode !== "fullscreen" && (
        <button
          ...
          className="absolute top-2 right-2 z-50 rounded-full bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 shadow-lg ring-1 ring-slate-900/10 dark:ring-white/10 p-2.5 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors cursor-pointer"
      ...
      <main className="flex flex-col gap-2">
        ...
        <div className={clsx("flex", displayMode === "inline" ? "w-full" : "")}>
          {displayMode === "fullscreen" && !!filters && (
          ...
          <PeopleTable
            ...
            className={clsx(
              displayMode === "inline" &&
                "rounded-lg max-h-[500px] overflow-auto w-full",
              displayMode === "fullscreen" && "rounded-none w-full"
            )}
          />
        </div>
        {/* <Image
